...
# Training
# ─────────────────────────────────────────
def train(speakers_train, speakers_val,speakers_test, base_dir, epochs, batch_size, lr):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    train_dataset = AVConcatDataset(speakers_train, base_dir)
    val_dataset   = AVConcatDataset(speakers_val,   base_dir)
    test_dataset  = AVConcatDataset(speakers_test,  base_dir)
    ...
    test_loader   = DataLoader(test_dataset,  batch_size=8, shuffle=False, collate_fn=collate_fn,num_workers=4, pin_memory=True)
    ...
    for epoch in range(epochs):
        ...
        for av, iam, mixed, clean in train_loader:
            ...
            loss = (    nn.L1Loss()(pred, target) +    nn.MSELoss()(torch.log(pred + 1e-8), torch.log(target + 1e-8)))
        ...
        test_loss=0.0
        with torch.no_grad():
            for av, iam, mixed, clean in test_loader:
                av, iam, mixed, clean = av.to(device), iam.to(device), mixed.to(device), clean.to(device)
                pred_iam   = model(av)
                pred_clean = pred_iam * mixed
                loss       = criterion(pred_clean, clean)
...
speakers_val  = ["s27","s28","s29","s30"]

speakers_test = ["s31","s32","s33","s34"]
epochs=60
batch_size=8
lr=1e-4
train(
    speakers_train ,
    speakers_val,   # 4 speakers

    speakers_test , # 4 speakers
    base_dir,
    epochs,
    batch_size,
    lr
