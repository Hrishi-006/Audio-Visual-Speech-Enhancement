...
def mix_audio(target, interferer, snr_db=0):
    """
    Mix target and interferer at given SNR (dB).
    Positive SNR -> interferer quieter
    Negative SNR -> interferer louder
    """

    target_rms = rms(target)
    interferer_rms = rms(interferer)

    # Desired interferer RMS based on SNR
